...
export async function fetch_user_data(targetUsername){
        const response = await fetch(`http://localhost:4747/api/get-user-count?username=${targetUsername}`,
        )
...
export async function post_friend_requests(sender, receiver){
    await fetch("http://localhost:4747/api/post-friend-requests",{
        method: "POST",
        credentials: "include",
        headers: {
                "Content-Type": "application/json",
...
export async function fetch_friend_requests(username){
    let data = await fetch(`http://localhost:4747/api/get-friend-requests?username=${username}`);
